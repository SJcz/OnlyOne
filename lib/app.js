...
class App extends events.EventEmitter {
    start(opts) {
        ...
        setInterval(() => {
            ...
            console.log(`进程 ${process.pid} 当前维持了 ${userIds.length} 个连接`);
        }, 5000);
    }
    ...
    _initEvents() {
        ...
        this.on('channel', (message) => {
            ...
            if (['chat', 'joinRoom', 'leavaRoom'].includes(message.route)) {
    ...
    async handleMessage(session, msg) {
        if (msg.route) {
            ...
            return session.send({type: 'response', code: 500, data: '找不到对应的 handler'});
        }
        session.send({type: 'response', code: 500, data: '服务器收到未知消息'});
    }
