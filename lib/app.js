        ...
        if (msg.route) {
            ...
            if (this.handlers[handlerName] && typeof this.handlers[handlerName][method] === 'function') {
                try {
                    ...
                    return session.send({ type: 'response', code: 0, data: result, requestId: msg.requestId });
                } catch (err) {
                    console.error(err);
                    return session.send({ type: 'response', code: 500, data: '服务器错误', requestId: msg.requestId });
                }
            }
            return session.send({ type: 'response', code: 404, data: '找不到对应的 handler', requestId: msg.requestId });
        }
        session.send({ type: 'response', code: 501, data: '服务器收到未知消息', requestId: msg.requestId });
    }
